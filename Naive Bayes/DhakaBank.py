...
ab = file.parse('Dhaka Bank')
zero=[ 0, 0, 0, 0, 0]

parameter = ["Deposits", "Loans and advances","Investments",  "NPL %", "Profit"]
...
for i in range (0,5):
    if((ab[2017][row[i]]-ab[2016][row[i]])>0):
        inp[i]=inp[i]+1
    else:
        inp[i]=inp[i]-1
...
for i in range (0,9):
    if((ab[2008+i][5]-ab[2007+i][5])>0):
        proInc=proInc+1
        for j in range (0,5):
            if(inp[j]>0):
                if((ab[2008+i][row[j]]-ab[2007+i][row[j]])>0):
...
for i in range (0,5):
        for j in range (0,9):
            #if(j!=1):
                if(inp[i]>0):
                    if((ab[2008+j][row[i]]-ab[2007+j][row[i]])>0):
